...
-- Part 3: Exploring Subqueries with ANY and ALL
    ...
    --Task 3.2: Products with Maximum Order Quantity
        ...
        select p.productName, sum(od.quantityOrdered) as total_order_quantity from orderdetails od join products p using(productCode) group by p.productName having total_order_quantity=all(select max(od.quantityOrdered) from orderdetails od);
